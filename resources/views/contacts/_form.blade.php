...
<div class="col-md-12">
    ...
    <div class="col-md-9">
        <select name="company_id" id="company_id" class="form-control @error('company_id') is-invalid @enderror">
            ...
            <option {{ $i === old('company_id', $contact->company_id) ? 'selected' : '' }} value="{{ $i }}">{{ $n }}</option>
            @endforeach
        </select>
        @error('company_id')
        <div class="invalid-feedback">
    ...
    <div class="col-md-9 offset-md-3">
        ...
        <a href="{{ $contact->exists ? route('contacts.show', $contact->id) : route('contacts.index') }}" class="btn btn-outline-secondary">Cancel</a>
    </div>
    </div>
</div>
</div>